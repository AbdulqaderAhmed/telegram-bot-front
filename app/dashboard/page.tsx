...
function StatusBadge({ status }: { status: ActivityLog['status'] }) {
  const map = {
    verified: { icon: CheckCircle2, cls: 'bg-blue-500/10 text-blue-500 border-blue-500/20' },
    pending:  { icon: Timer,        cls: 'bg-amber-500/10 text-amber-500 border-amber-500/20' },
    left:     { icon: XCircle,      cls: 'bg-foreground/10 text-foreground/50 border-foreground/10' },
  };
  const { icon: Icon, cls } = map[status];
  return (
    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 border w-fit ${cls}`}>
...
export default function DashboardOverview() {
  const { userStats, referralStats, activity, isLoading, lastUpdated, error, fetchAll, startPolling } = useDashboardStore();
  ...
  const stats = [
    ...
    },
    {
      ...
      change: referralStats ? `${referralStats.verified}/${referralStats.total}` : '…',
      icon: Target,
      color: 'amber',
    },
    {
    ...
    <div className="space-y-10">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-4xl font-black text-foreground tracking-tight">Ecosystem Overview</h1>
          <p className="text-foreground/60 font-medium">Real-time health synchronization & referral metrics.</p>
        </div>
        ...
        {(isLoading && !userStats) ? (
          ...
          stats.map((stat, i) => (
            <motion.div
              ...
              className="p-6 glass rounded-3xl group cursor-pointer hover:border-foreground/20 transition-all hover:translate-y-[-4px]"
            >
              <div className="flex items-center justify-between mb-6">
                <div className={`p-3 rounded-2xl bg-${stat.color}-500/10 text-${stat.color}-600 group-hover:bg-${stat.color}-500 group-hover:text-white transition-all`}>
              ...
              <p className="text-foreground/60 text-sm font-bold uppercase tracking-tight mb-2">{stat.label}</p>
              <h3 className="text-3xl font-black text-foreground font-mono">{stat.value}</h3>
            </motion.div>
          ))
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glass rounded-[2.5rem] p-8 border border-foreground/10 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-10">
            ...
            {isLoading && <RefreshCw className="w-4 h-4 text-foreground/40 animate-spin" />}
          </div>

          {referralStats && referralStats.total > 0 ? (
            <div className="space-y-6">
              {[
                { label: 'Verified', value: referralStats.verified, total: referralStats.total, color: 'bg-blue-500' },
                { label: 'Pending', value: referralStats.pending, total: referralStats.total, color: 'bg-amber-500' },
                { label: 'Left', value: referralStats.left, total: referralStats.total, color: 'bg-foreground/20' },
              ].map((row) => {
                ...
                return (
                  <div key={row.label} className="space-y-2">
                    <div className="flex items-center justify-between text-sm font-bold">
                      ...
                      <span className="text-foreground font-mono">{row.value.toLocaleString()} <span className="text-foreground/40">({pct}%)</span></span>
                    </div>
              ...
              <div className="grid grid-cols-3 gap-4 mt-8 pt-8 border-t border-foreground/10">
                <div className="text-center">
                  <p className="text-3xl font-black text-foreground font-mono">{referralStats.conversionRate}%</p>
                  <p className="text-xs font-bold text-foreground/50 uppercase tracking-widest mt-1">Conversion</p>
                </div>
                <div className="text-center">
                  <p className="text-3xl font-black text-foreground font-mono">{referralStats.total}</p>
                  <p className="text-xs font-bold text-foreground/50 uppercase tracking-widest mt-1">Total</p>
                </div>
                <div className="text-center">
                  <p className="text-3xl font-black text-foreground font-mono">{referralStats.pending}</p>
                  <p className="text-xs font-bold text-foreground/50 uppercase tracking-widest mt-1">Pending</p>
                </div>
              </div>
            </div>
          ) : (
            <div className="h-[250px] flex items-center justify-center text-foreground/30 font-bold">
        ...
        {/* Live Activity Feed */}
        <div className="glass rounded-[2rem] p-8 border border-foreground/10 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <h4 className="text-foreground font-bold text-sm uppercase tracking-widest">Live Activity</h4>
          ...
          <div className="space-y-3 flex-1 overflow-hidden">
            {isLoading && !activity.length ? (
              ...
              <div className="h-full flex items-center justify-center text-foreground/30 text-sm font-bold">No activity yet</div>
            ) : (
              activity.map((log, i) => (
                <motion.div
                  key={log.id}
                ...
                >
                  <div className="w-8 h-8 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-[10px] font-black text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-all shrink-0">
                    {(log.referrer?.username || log.referrer?.telegramId || '?').charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-bold text-foreground truncate">
                      @{log.referrer?.username || log.referrer?.telegramId || '—'} <ArrowRight className="inline w-3 h-3" /> @{log.referredUser?.username || log.referredUser?.telegramId || '—'}
                    </p>
          ...
          <div className="pt-6 border-t border-foreground/10">
            <div className="flex items-center gap-3 p-4 rounded-2xl bg-blue-500/5 border border-blue-500/10 hover:border-blue-500/20 transition-all group cursor-pointer">
              ...
              <div className="flex-1">
                <p className="text-foreground font-bold text-sm">Sync HRIS Data</p>
                <p className="text-foreground/50 text-[10px] font-medium">
                  {userStats ? `${userStats.totalHris} users synced` : 'Checking…'}
                </p>
              </div>
            </div>
          </div>
